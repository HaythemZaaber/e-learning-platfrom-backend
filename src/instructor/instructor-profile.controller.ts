...
export class InstructorProfileController {
  ...
  @ApiOperation({
    summary: 'Get comprehensive instructor details for public profile page',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiQuery({
    name: 'status',
    required: false,
    description: 'Course status filter (DRAFT, PUBLISHED, ARCHIVED, etc.)',
  })
  ...
  async updateAutoApprovalSettings(
    ...
    return this.instructorProfileService.updateAutoApprovalSettings(
      userId,
      body.autoAcceptBookings,
    );
  }
  ...
  async getInstructorAvailability(
    ...
    return this.instructorProfileService.getInstructorAvailability(
      instructorId,
      {
